...
namespace Truesoft.Supabase.Unity
{
    ...
    public static class RemoteConfig<T> where T : class, new()
    {
        ...
        /// </summary>
        /// <param name="maxStaleSeconds">유효로 간주할 최대 캐시 경과 시간(초). 0이면 DB 설정을 따릅니다.</param>
        /// <example><code>
        /// var cfg = await RemoteConfig&lt;GameplayV1Config&gt;.CreateReader()();
        /// </code></example>
        public static Func<Task<T>> CreateReader(int maxStaleSeconds = 0) =>
            SupabaseSDK.CreateRemoteConfigReader<T>(GetKey(), maxStaleSeconds);
        ...
        /// </summary>
        /// <param name="pollIntervalSeconds">갱신 확인 주기(초). 0이면 자동 폴링 없음.</param>
        /// <example><code>
        /// _binding = RemoteConfig&lt;GameplayV1Config&gt;.CreateBinding();
        /// var cfg = _binding.Value;
        /// </code></example>
        public static RemoteConfigBinding<T> CreateBinding(float pollIntervalSeconds = 60f) =>
            SupabaseSDK.CreateRemoteConfigBinding<T>(GetKey(), pollIntervalSeconds);
        ...
        public static RemoteConfigListener<T> CreateListener(
            Action<T> onChange, float pollIntervalSeconds = 60f) =>
            SupabaseSDK.CreateRemoteConfigListener<T>(GetKey(), pollIntervalSeconds, onChange);
    }
}
