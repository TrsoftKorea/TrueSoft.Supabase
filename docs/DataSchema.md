...
| 파일 | 내용 |
|------|------|
| `01_servers.sql` | game_servers, ts_default_server_id, ts_server_now |
| `02_profiles.sql` | user_profiles, display_names, user_sessions |
| `03_anonymous_recovery.sql` | 익명 복구 토큰 + auth 트리거 |
| `04_user_data.sql` | set_updated_at, ts_ensure_my_row, user_data 테이블 + 필드 보호 |
| `05_account_management.sql` | 서버 이주 RPC, 탈퇴 RPC |
| `06_mails.sql` | mails 테이블 + RPC (hardening 포함) |
| `07_purchases.sql` | IAP 구매 검증 기록 |
| `08_remote_config.sql` | remote_config 테이블 |
| `09_cron_jobs.sql` | withdrawal_delete_queue + 크론 잡 (pg_cron 필요) |
...
| 구분 | 처리 |
|------|------|
| 운영 데이터 (`user_profiles`, `user_data`, `chat_messages` 등) | 탈퇴 완료 시 삭제 또는 비식별화 |
