...
| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `productIds` | `string[]` | 등록할 소모품 ID 목록 |
| `onGrant` | `Func<string, bool, bool, Task<bool>>` | 서버 검증 완료 후 아이템 지급 콜백. `true` 반환 시 SDK가 소비 처리 |
| `onFailed` | `Action<IAPPurchaseFailedInfo>` | 구매 실패 콜백 |
| `timeoutMs` | `int` | 초기화 대기 최대 시간 ms (기본 10초) |
