...
| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `displayName` | `string` | 확인하거나 설정할 닉네임. 최대 64자 (초과 시 자동으로 잘림) |
| `userId` | `string` | 조회할 플레이어의 ID |
| `defaultValue` | `string` | 조회 실패 시 반환할 기본값 |

- `TryIsDisplayNameAvailableAsync` — 닉네임 사용 가능 여부를 서버에서 확인합니다. 현재 내 닉네임은 항상 사용 가능으로 반환됩니다.
- `TrySetMyDisplayNameAsync` — 내 닉네임을 설정합니다.
- `TryGetPublicDisplayNameAsync` — 다른 플레이어의 닉네임을 조회합니다. 리더보드·매칭 결과에서 얻은 `userId`를 전달합니다.
...
| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `defaultValue` | `string` | 토큰 발급 실패 시 반환할 기본값 |
| `cancelToken` | `string` | 플레이어가 입력한 취소 토큰 |
...
| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `targetServerCode` | `string` | 이주할 서버 코드 (예: `"GLOBAL"`, `"KR1"`) |
| `reason` | `string` | 이주 사유 (서버 로그용) |
