...
///
/// 리더보드 생성은 운영(Retool) 전용입니다 — Retool의 리더보드 페이지에서 코드 <c>arena</c>로 먼저 만들고,
/// <c>TrueSoft > Supabase > 클래스 생성 > 리더보드</c>로 타입을 생성하세요.
/// 이 샘플은 그 생성 결과와 같은 형태의 <see cref="ArenaLeaderboard"/>를 씁니다.
///
...
public sealed class SampleLeaderboard : MonoBehaviour
{
    ...
    [Tooltip("켜면 점수와 함께 ArenaLeaderboard.Row(char_level)를 보냅니다. Retool에 필드를 등록하지 않았다면 끄세요.")]
    [SerializeField] private bool sendPlayerData = true;
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha5)) _ = ShowRanksAsync(null);
    ...
    private async Task ListLeaderboardsAsync()
    {
        var r = await Supabase.GetLeaderboardsAsync();
    ...
    {
        var r = await Supabase.GetLeaderboardAsync<ArenaLeaderboard>();
    ...
    private async Task SubmitAsync()
    {
        ...
        // 필드를 함께 보낼 때만 Row를 넘깁니다. 어느 리더보드인지는 Row 타입에서 읽습니다.
        var r = sendPlayerData
            ? await Supabase.SubmitScoreAsync(score, new ArenaLeaderboard.Row { CharLevel = Random.Range(1, 100) })
            : await Supabase.SubmitScoreAsync<ArenaLeaderboard>(score);
    ...
    private async Task ShowRanksAsync(int? rotationCount)
    {
        var r = await Supabase.GetRanksAsync<ArenaLeaderboard>(1, 10, rotationCount);
        ...
        {
            // 추가 데이터는 생성 타입의 Row로 되돌려 씁니다(네트워크 호출 없음).
            var row = Supabase.ToRow<ArenaLeaderboard.Row>(e);
            Debug.Log($"{Tag}  {e.Rank}위 {e.DisplayName ?? "(이름 없음)"} — {e.Score} (레벨 {row.CharLevel})");
        }
    }
    ...
    {
        var r = await Supabase.GetRankAsync<ArenaLeaderboard>();
    ...
    {
        var table = await Supabase.GetLeaderboardAsync<ArenaLeaderboard>();
    ...
    private async Task EditMyDataAsync()
    {
        if (!sendPlayerData)
        {
            Debug.Log($"{Tag} 플레이어 데이터를 쓰지 않는 설정입니다. Inspector에서 켜세요.");
            return;
        }
    ...
    {
        var r = await Supabase.DeleteMyScoreAsync<ArenaLeaderboard>();
