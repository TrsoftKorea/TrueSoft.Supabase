...
- 유저 세이브 로드/저장 예시 (`SampleUserSave : StaticUserSave<SampleUserSave.Row>` — `TryLoadAsync`로 서버 데이터 로드, 신규 유저는 자동으로 행 생성 후 DB 기본값 반환, `TrySaveIfChangedAsync`로 diff만 PATCH)
...
8. (선택) `SampleUserSave.Row`의 `[DataTable("basic")]`을 실제 테이블명으로 변경하고, 테이블에 `level int`, `coins int`, `updated_at timestamptz` 컬럼이 있어야 합니다. 테이블은 `admin_create_user_table` RPC로 생성하면 필수 구조(`account_id unique` 포함)가 자동 적용됩니다. 신규 유저 첫 로드 시 행이 없으면 자동 생성 후 DB 기본값을 반환합니다. **Run Load User Save Example** 또는 키 **R**: `TryLoadAsync`로 서버 데이터 로드. **Run Save User Save Example** 또는 키 **V**: 로드로 스냅샷을 맞춘 뒤 인스펙터 값을 반영하고 `TrySaveIfChangedAsync`로 **서버와 다른 컬럼만** PATCH합니다(같으면 요청 없음). 쿨타임 자동 저장은 **Supabase 런타임**(`SupabaseRuntime`)과 프로퍼티 setter의 `MarkDirty` 경로를 쓰면 됩니다.
