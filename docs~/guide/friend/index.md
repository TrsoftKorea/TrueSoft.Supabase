...
## 마지막 활동 시각 {#last-activity}

친구 목록·요청 목록·닉네임 검색 결과에는 상대의 `LastActivityAt`이 함께 옵니다. 갱신되는 시점은 **둘뿐**입니다.

- 그 사람이 로그인할 때
- 그 사람의 유저 데이터가 저장될 때

그래서 앱을 켜 둔 채 저장이 일어나지 않으면 값이 움직이지 않습니다. **"지금 접속 중"을 뜻하지 않으므로 접속 중 표시에는 쓰지 마세요** — 그 용도는 별도 설계가 필요합니다. 기록이 없으면 `null`입니다.
