...
메일은 `category`(분류) 값으로 나뉩니다. 기본값은 `default`이며, 어드민이 발송 시 자유 텍스트로 지정합니다. 별도의 분류 관리 테이블은 없습니다 — 발송자가 정한 문자열을 그대로 씁니다. 공지·이벤트·보상처럼 소수의 고정된 값으로 운영하는 것을 권장합니다.
...
| 메서드 | 설명 |
|--------|------|
| [`GetMyMailsAsync`](/guide/mailbox/list) | 우편함 목록 조회(분류 필터) |
| [`GetMailDetailAsync`](/guide/mailbox/detail) | 우편 상세 조회 |
