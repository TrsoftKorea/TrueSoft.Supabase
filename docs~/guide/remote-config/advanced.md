...
3. 키 선택 후 **필드 파싱** — DB의 JSON 구조가 필드 목록으로 표시됩니다(확인용 읽기 전용)
4. 타입을 바꾸려면 **CSV 내보내기 → 편집 → CSV 불러오기**로 반영합니다 (`int` → `long`, `string` → `List<string>` 등)
5. 클래스명 확인 (키 이름에서 자동 유도, 수정 가능)
6. **소스 생성** → **저장**으로 `.cs` 파일을 프로젝트에 저장합니다

CSV의 열은 `field`(필드 경로 — 매칭 키이므로 수정 금지) · `type` · `include`(`1`/`0`)입니다. 중첩 객체 행의 `type`은 `(중첩 객체)`로 표시되며 수정할 수 없습니다. 불러오기 시 타입 해석을 검사해, 오타·없는 타입이면 ✕(빨강)로 표시되고 해결 전까지 소스 생성이 차단됩니다. [유저 데이터 클래스의 CSV 편집](../user-data/class-gen#csv-edit)과 같은 방식입니다.
