...
CSV의 열은 `field` · `type` · `include`입니다. `field`는 필드 경로로, 매칭 키이므로 수정하면 안 됩니다. `include`는 `1`이 포함, `0`이 제외입니다. 중첩 객체 행의 `type`은 `(중첩 객체)`로 표시되며 수정할 수 없습니다. 불러오기 시 타입 해석을 검사해, 오타·없는 타입이면 빨간 ✕로 표시되고 해결 전까지 소스 생성이 차단됩니다. [유저 데이터 클래스의 CSV 편집](../user-data/class-gen#csv-edit)과 같은 방식입니다.
