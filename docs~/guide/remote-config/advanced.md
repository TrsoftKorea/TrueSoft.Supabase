...
중첩 클래스를 포함한 설정 클래스는 매개변수 없는 생성자로 역직렬화됩니다. 생성자를 직접 정의했다면 기본 생성자도 함께 두면 됩니다. 생성기로 만들면 자동으로 처리됩니다.
...
1. 메뉴 **TrueSoft > Supabase > 클래스 생성 > 원격 설정** 열기. Secret 키는 창 상단에서 바로 입력합니다
2. **키 목록 가져오기** — `remote_config` 테이블의 키 목록이 드롭다운으로 표시됩니다
3. 키 선택 후 **필드 목록 가져오기** — DB의 JSON 구조가 필드 목록으로 표시됩니다
...
CSV의 열은 `필드` · `타입` · `포함`입니다. `필드`는 필드 경로로, 매칭 키이므로 수정하면 안 됩니다. `포함`은 `1`이 포함, `0`이 제외입니다. 중첩 객체 행의 `타입`은 `(중첩 객체)`로 표시되며 수정할 수 없습니다. [유저 데이터 클래스의 CSV 편집](../user-data/class-gen#csv-edit)과 같은 방식입니다.
