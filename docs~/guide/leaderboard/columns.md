...
순위에 닉네임·점수 말고 레벨이나 길드명 같은 값을 함께 보여주고 싶을 때 씁니다. 유저 데이터처럼 **컬럼을 지정**해 두고, 점수를 기록할 때 값을 함께 보냅니다.

## 컬럼은 리더보드마다 따로 정합니다 {#per-leaderboard}

운영자가 Retool의 **리더보드 > 컬럼** 탭에서 리더보드마다 사용할 컬럼을 등록합니다. 등록하지 않은 컬럼을 보내면 기록이 거부되므로, 리더보드마다 필요한 값만 주고받습니다.
...
순위·플레이어 조회 결과의 `.Data`에 등록된 컬럼만 담겨 옵니다.
...
메뉴에서 **TrueSoft > Supabase > 리더보드 클래스 생성**을 열고, 리더보드 코드와 컬럼 이름을 입력하면 해당 컬럼만 담은 클래스가 생성됩니다.
...
리더보드마다 별도 클래스가 생성되므로 서로의 컬럼이 섞이지 않습니다.

::: warning 컬럼 추가는 운영자만
컬럼을 새로 만들거나 지우는 것은 Retool에서만 가능합니다. 게임에서는 이미 등록된 컬럼에 값을 넣고 읽을 수만 있습니다.
:::
