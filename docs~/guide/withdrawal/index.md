...
1. **탈퇴 신청** — `RequestMyWithdrawalAsync()`로 유예를 예약합니다. 성공하면 **즉시 로그아웃**되어 본편에 들어갈 수 없습니다.
2. **유예 중 재로그인** — 예약된 계정으로 다시 로그인하면 SDK가 **게이트**에서 취소 토큰을 자동 발급·저장하고 세션을 정리한 뒤, 로그인을 `WithdrawalGateBlocked` 실패로 반환합니다.
3. **탈퇴 취소** — 그 실패를 감지해 `RedeemWithdrawalCancelAsync()`를 호출하면 예약이 해제됩니다.
4. **유예 만료(완료)** — 취소하지 않고 유예가 지나면 다음 로그인 시 계정이 실제로 삭제됩니다. 완료된 탈퇴는 되돌릴 수 없으며, 재로그인 시 새 계정이 생성됩니다.
