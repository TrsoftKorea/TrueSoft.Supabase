...
**읽기 실패와 데이터 없음을 구분합니다.** 플레이나누 스토리지를 읽지 못한 로그인에서는 위 표의 첫 줄로 빠져 아무것도 하지 않고, 그 로그인 동안 플레이나누 쓰기를 막습니다. 다음 로그인에서 다시 맞춥니다.

**서버 정본을 못 읽었으면 쓰지 않습니다.** `SaveCurrentToNanoo()`는 로컬이 서버 데이터를 반영한 뒤에만 동작합니다. 로그인·동기화가 끝나기 전에 부르면 경고만 남기고 넘어갑니다 — 그 시점 로컬은 기본값이라 내보내면 원본이 지워집니다.
