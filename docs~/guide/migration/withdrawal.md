...
탈퇴 예약과 취소 흐름은 SDK 단독 사용과 동일합니다. 유예 기간 중의 **예약 취소**이며, 유예가 지나 탈퇴가 완료(계정 삭제)되면 되돌릴 수 없습니다.
...
| Google / Apple | `OnWithdrawalCancelled` 이벤트 발행 → 개발자가 재인증 UI 표시 후 `ClearMyWithdrawalAsync()` 직접 호출 |
