...
먼저 [대시보드 설정](./setup)을 완료하세요. [신규 로그인](./signin)(`TrySignInWithAppleAsync`)은 **iOS·Android를 자동으로 처리**하므로 호출부는 플랫폼을 구분할 필요가 없습니다. 이미 가진 Apple ID 토큰을 직접 쓸 땐 커스텀 메서드를 사용합니다.
