...
스키마를 가져오면 컬럼이 표로 나옵니다. **표는 확인용(읽기 전용)이며, 편집은 CSV로 합니다**(아래 절차). 컬럼이 많으면 표 위 **검색** 칸으로 이름을 필터링할 수 있습니다(표시만 거를 뿐 생성 대상엔 영향 없음).

| 칸 | 설명 |
|----|------|
| **컬럼** | DB 컬럼명. `[DataColumn]` 매핑 키로 그대로 쓰입니다. 상태에 따라 ⚠(타입 미지정)·✕(타입 해석 실패)가 붙습니다. |
...
| **포함** | 해제된 컬럼은 흐리게 표시되고 생성에서 제외됩니다. |
...
1. 스키마를 가져온 뒤 **CSV 내보내기**로 현재 설정을 파일로 저장합니다. **CSV 열기**를 누르면 기억된 파일이 기본 편집기로 열립니다(파일이 없으면 먼저 내보낸 뒤 엽니다).
2. 엑셀·구글시트에서 편집합니다.
...
불러오기 시 `type` 열의 타입이 실제로 해석 가능한지 검사합니다. 오타나 없는 타입이면 해당 컬럼이 목록에 ✕(빨강)로 표시되고 팝업으로 안내되며, **해결 전까지 소스 생성이 차단**됩니다 — 컴파일 에러를 CSV 단계에서 미리 잡습니다.

::: tip 커스텀 타입 지정
`type` 열에는 스칼라·컬렉션 외에 enum 키나 커스텀 클래스 값도 쓸 수 있습니다(예: `Dictionary<HeroName, HeroData>`). 생성 클래스와 다른 네임스페이스의 타입은 정규화 이름(`MyGame.HeroName`)으로 쓰세요. enum 키는 멤버 **이름**이 DB에 저장되므로, 이름 변경·삭제는 저장 데이터 마이그레이션이 필요합니다.
:::
...
jsonb 컬럼은 가져올 때 `Dictionary<string, object>`(미지정)로 시작하며 ⚠ 로 표시되고, 그 상태로는 **소스 생성이 막힙니다**. CSV의 `type` 열에서 타입을 좁히면(예: `Dictionary<string, int>`) 풀립니다.
:::
