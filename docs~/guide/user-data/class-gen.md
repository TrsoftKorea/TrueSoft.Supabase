...
`SupabaseSettings` 에셋 Inspector 하단에서 Secret 키를 입력하고 **필드 목록 가져오기 → 소스 생성 → 저장**으로 `PlayerSave.cs`를 생성합니다. 타입·필드명·기본값 설정은 재생성 시 컬럼 기준으로 보존되고, 기존 클래스가 있으면 그 파일에 바로 저장됩니다.
...
포함된 모든 컬럼은 `default`를 반드시 채워야 합니다. 비어 있으면 인스펙터 표의 기본값 칸에 `⚠ 필요`가 표시되고 **소스 생성이 차단**됩니다. 스칼라는 `0`·`false`·문자열 같은 시작값을, 값 컬렉션은 슬롯 기본값을, 클래스 요소 컬렉션은 생성자 인자를 씁니다. 빈 문자열을 기본값으로 두려면 `""`처럼 명시하세요. (`account_id`·`user_id`·`updated_at` 등 시스템 컬럼은 애초에 목록에서 제외됩니다.)
:::
