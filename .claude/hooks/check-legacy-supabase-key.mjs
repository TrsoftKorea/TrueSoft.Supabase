...
// 조용히 반복해서 쓰게 된다. 이 프로젝트는 이미 secret key(SUPABASE_SECRET_KEY(S))를 쓰고
// 있으니, 이 훅은 앞으로 실수로 legacy 로 되돌아가는 것만 막는다.
//
// **SQL의 `service_role` 권한 이름은 건드리지 않는다** — grant/revoke·has_*_privilege 는
// 정당한 기존 패턴이다(README.md 등). 여기서 잡는 건 백엔드 코드가 연결에 쓰는 키 자료뿐이다.
