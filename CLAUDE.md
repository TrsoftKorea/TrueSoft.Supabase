...
API 색인이 링크하는 기능 페이지들은 **어느 페이지를 열어도 같은 구조**여야 한다.

1. **머리**: `# 기능명` → **1~2줄 도입문**(페이지 주제에 한정, "인증 전반" 같은 광역 설명 금지) → 선택적 `:::` 콜아웃. 도입문 없이 바로 `##`나 시그니처로 진입하지 않는다.
2. **함수 구분**: 함수가 2개 이상이면 각각 `## `로 분리하고 **함수 사이에 `---` 구분선을 쓰지 않는다**(`## `가 구분 역할). 단일 함수면 `##` 없이 도입문 직후 함수 블록.
3. **함수 블록 순서 고정** (Rule 8): 헤딩 → ```csharp 시그니처(`반환타입 메서드(...)`, `public`/`static`/`async` 등 수식어 제외) → 한 줄 설명 → 파라미터 표 → 반환 → 실패 원인.
4. 한 페이지 안에서 함수마다 같은 요소(파라미터/반환/실패 원인)는 **있으면 모두, 없으면 모두** 일관되게 — 일부 함수만 실패 원인 표가 있고 일부는 없는 식의 들쭉날쭉을 피한다.
